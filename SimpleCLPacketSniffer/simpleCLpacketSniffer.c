...
Link-layer type:
	...
	104		Cisco HDLC
	105		802.11
	108		later OpenBSD loopback devices (with the AF_value
	...
	if(quiet){ // if quiet equal to 1 = display
		...
		if(eth_contains_ip(eth_pkt)){
			...
			if(tcph){
				...
				printf(",sport=%d,dport=%d", ntohs(tcph->srcPort), ntohs(tcph->dstPort));
			}
			else if (udph){
...
//			pcap_pkt_sleep(&curtime, &lasttime);
		...
		if( bytes_read > 0 ){
			...
			if(oface && od && res == 1){
				...
				sendto(od, data, bytes_read, 0, (struct sockaddr *)&peerAddr, sizeof(peerAddr));
			}
